...
app.get("/api/redir", (req, res) => {
  ...
  return request({
    ...
    uri:
      ...
      ip +
      "?access_key=7a580c7a660c0e54158725e9c8aaff6d",
    json: true,
...
function convLocalLink(link, country, type) {
  if (country === "US" || (country !== "US" && country !== "CA")) {
    if (type === "2415") {
      link =
        "https://www.amazon.com/Canon-24-105mm-USM-Zoom-Lens/dp/B000B84KAW/ref=sr_1_3?ie=UTF8&qid=1518289159&sr=8-3&tag=devoncrawfo05-20";
    } else {
...
app.get("/api/yt-auth", (req, res) => {
  ...
  const uri = (auth_uri, scope, access, youtubeRedirect, clientId) => {
    return (
      `${auth_uri}?scope=${scope}&access_type=${access}&include_granted_scopes=true&state=` +
  ...
  if (typeof authCode === "undefined" || !authCode) {
    ...
    res.redirect(
      uri(json.auth_uri, scope, access, youtubeRedirect, json.client_id)
    );
  } else {
    request.post(
      {
...
app.get("/api/yt-desc/uploads/:range/:nextPageToken/:all", (req, res) => {
  ...
  request(
    ...
    function (error, response, body) {
      ...
      for (
        let i = 0;
        i < range && youtubeUploads.length < req.params.all;
        i++
      ) {
      ...
      if (
        youtubeUploads.length === req.params.all ||
        typeof json.nextPageToken === "undefined"
      ) {
...
app.get(
  "/api/yt-desc/uploadRange/:startId/:endId/:nextPageToken",
  (req, res) => {
    let params = {
      access_token: youtubeToken,
      part: "snippet,contentDetails,status",
    ...
    request(
      ...
      (error, response, body) => {
        ...
        if (endFlag || typeof json.nextPageToken === "undefined") {
          ...
          res.redirect(
            `/api/yt-desc/uploadRange/${req.params.startId}/${req.params.endId}/${json.nextPageToken}`
          );
      }
    );
  }
